...
export async function downloadSessionJson(id: string): Promise<void> {
  const res = await fetch(`${API_URL}/api/sessions/${id}/download`, {
    headers: authHeaders(),
  })
  if (!res.ok) return
  const blob = await res.blob()
  const disposition = res.headers.get('Content-Disposition') || ''
  const match = disposition.match(/filename="([^"]+)"/)
  const filename = match ? match[1] : `session_${id.slice(0, 8)}.json`
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a')
  a.href = url
  a.download = filename
